...
public class Round {
    ...
    private final ShootLogic logic;
    ...
    private ShootLogic choseLogic() {
        ShootLogic logic;
        if (game.getSettings().getLevelOfDifficulty() == Level.LOW) {
            logic = new LogicLowLevel(game.getGamerBattleField());
        } else if (game.getSettings().getLevelOfDifficulty() == Level.MEDIUM) {
            logic = new LogicMediumLevel(game.getGamerBattleField());
        } else if (game.getSettings().getLevelOfDifficulty() == Level.HIGH) {
            logic = new LogicHighLevel(game.getGamerBattleField());
        } else {
            logic = new LogicExpertLevel(game.getGamerBattleField());
        }
        return  logic;
    }
    ...
    private boolean updateCompMove(SequentialTransition animPlay) {
        ...
        Point shootPoint = logic.getNextShoot();
        ...
        if (checkForGamerWin(game.getGamerShips())) {
